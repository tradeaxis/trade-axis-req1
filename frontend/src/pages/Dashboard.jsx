...
const Dashboard = () => {
  ...
  const currentQuoteTimestamp = Number(currentQuote?.timestamp || 0);
  const currentQuoteHasPrice =
    Number(currentQuote?.bid || 0) > 0 ||
    Number(currentQuote?.ask || 0) > 0 ||
    Number(currentQuote?.last || currentQuote?.last_price || 0) > 0;
  const selectedSymbolOffQuotes =
    !!selectedSymbol &&
    isMarketOpenNow(selectedSymbol) &&
    (
      !currentQuoteHasPrice ||
      !!currentQuote?.off_quotes ||
      !currentQuoteTimestamp ||
      (Date.now() - currentQuoteTimestamp > QUOTE_STALE_MS)
    );

  const bid = Number(currentQuote?.bid || currentQuote?.last || currentQuote?.last_price || 0);
  const ask = Number(currentQuote?.ask || currentQuote?.last || currentQuote?.last_price || 0);
  ...
  const desktopMarketSymbols = useMemo(() => {
    void staleTick;

    return (symbols || []).map((sym) => {
      const quote = quotes?.[sym.symbol];
      ...
      const liveBid = Number(quote?.bid ?? sym.bid ?? sym.last_price ?? 0);
      const liveAsk = Number(quote?.ask ?? sym.ask ?? sym.last_price ?? 0);
      const liveLast = Number(quote?.last ?? sym.last_price ?? 0);
      const offQuotes =
        marketOpen &&
        isSubscribed &&
        (
          !!quote?.off_quotes ||
          !(liveBid > 0 || liveAsk > 0 || liveLast > 0) ||
          !quoteTimestamp ||
      ...
      return {
        ...
        ask: liveAsk,
        last: liveLast,
        change: Number(quote?.change ?? quote?.change_value ?? sym.change_value ?? 0),
        change_percent: Number(quote?.change_percent ?? sym.change_percent ?? 0),
        timestamp: quoteTimestamp || sym.timestamp || 0,
        off_quotes: offQuotes,
      };
    });
  }, [symbols, quotes, activeSymbols, staleTick]);
...
const placeOrderWithQty = async (type, qty, execType = 'instant', execPrice = 0) => {
    ...
    if (
      ...
      currentQ.off_quotes ||
      !currentQ.timestamp ||
      (Date.now() - Number(currentQ.timestamp || 0) > QUOTE_STALE_MS)
    ) {
    ...
    const currentBid = Number(currentQ?.bid || currentQ?.last || 0);
    const currentAsk = Number(currentQ?.ask || currentQ?.last || 0);
    const hasPriceData = currentQ && (currentBid > 0 || currentAsk > 0);
    const isOffQuotes =
      !hasPriceData ||
      !!currentQ?.off_quotes ||
      !quoteTs ||
      (Date.now() - quoteTs > QUOTE_STALE_MS);
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingAllSymbols && allFuturesSymbols.length === 0 ? (
            ...
            quotesDisplayedSymbols.map((sym) => {
              ...
              const symLast = Number(quote.last || quote.last_price || sym.last_price || 0);
              ...
              // 3. Quote timestamp is older than 10 seconds
              const isSubscribed = (activeSymbols || []).includes(String(sym.symbol).toUpperCase());
              const hasQuoteData = symBid > 0 || symAsk > 0 || symLast > 0;
              const isStale =
                marketOpen &&
                isSubscribed &&
                (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                disabled={selectedSymbolOffQuotes}
                className="py-3.5 rounded-lg font-bold text-white text-lg"
                style={{
                  background: '#ef5350',
                  opacity: selectedSymbolOffQuotes ? 0.6 : 1,
                  cursor: selectedSymbolOffQuotes ? 'not-allowed' : 'pointer',
                }}
              >
                SELL {bid.toFixed(2)}
              </button>
        ...
        <DesktopTerminal
          ...
          right={
            <OrderDockPanel
              ...
              bid={bid}
              ask={ask}
              offQuotes={selectedSymbolOffQuotes}
