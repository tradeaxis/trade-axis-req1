...
class TradingService {
  ...
  async resolveCloseMarketData(trade, overrideClosePrice = null) {
    ...
    const { data: symbolData, error } = await supabase
      .from('symbols')
      .select('symbol, bid, ask, last_price, close_price, previous_close, last_update, lot_size')
  ...
  async checkPendingOrders() {
    try {
      ...
      const { data: symbolRows } = await supabase
        .from('symbols')
        .select('symbol, exchange, bid, ask, last_price, close_price, previous_close, last_update, lot_size, tick_size, display_name')
  ...
  async checkStopLossAndTakeProfit() {
    try {
      ...
      const { data: symbolRows } = await supabase
        .from('symbols')
        .select('symbol, exchange, bid, ask, last_price, close_price, previous_close, last_update')
        .in('symbol', symbolsToResolve);
