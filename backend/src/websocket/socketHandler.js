...
class SocketHandler {
  ...
  handleSubscribe(socket, symbols) {
    ...
    symbols.forEach((sym) => {
      ...
      if (cached) {
        const isFresh = kiteStreamService.isPriceFresh(s);
        snapshot.push({
          ...
          timestamp: cached.timestamp, source: isFresh ? 'kite' : 'stale_kite',
          off_quotes: !isFresh,
        });
      }
    });
    ...
    this.pnlUpdateInterval = setInterval(async () => {
      try {
        ...
        for (const sym of uniqueSymbols) {
          const c = kiteStreamService.getFreshPrice(sym);
        ...
        for (const trade of openTrades) {
          ...
          if (!p) {
            const storedCurrentPrice = Number(trade.current_price || trade.open_price || 0);
            const storedProfit = Number(trade.profit || 0);

            accountPnL[trade.account_id].totalPnL += storedProfit;
            accountPnL[trade.account_id].trades.push({
              ...trade,
              _currentPrice: storedCurrentPrice,
              _netPnL: storedProfit,
            });
            continue;
          }
